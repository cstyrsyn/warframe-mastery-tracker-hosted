#!/usr/bin/env node
// Downloads arch-weapon images from the wiki into Images/arch-weapons/
// Run once: node download-arch-weapon-images.js
...
const OUT_DIR = path.join(__dirname, '..', '..', 'Images', 'arch-weapons');
fs.mkdirSync(OUT_DIR, { recursive: true });

const dataJs        = fs.readFileSync(path.join(__dirname, '..', '..', 'data.js'), 'utf8');
const archWeapBlock = dataJs.match(/const ARCH_WEAPONS\s*=\s*\[([\s\S]*?)\];/)?.[1] ?? '';
const names         = [...new Set([...archWeapBlock.matchAll(/\["([^"]+)"/g)].map(m => m[1]))];

if (names.length === 0) {
  console.error('No arch-weapon names found in data.js — check the regex.');
  process.exit(1);
}
console.log(`Found ${names.length} arch-weapons.\n`);
