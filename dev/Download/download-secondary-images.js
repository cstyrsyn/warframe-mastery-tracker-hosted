#!/usr/bin/env node
// Downloads secondary weapon images from the wiki into Images/secondary/
// Run once: node download-secondary-images.js
...
const OUT_DIR = path.join(__dirname, '..', '..', 'Images', 'secondary');
fs.mkdirSync(OUT_DIR, { recursive: true });

const dataJs         = fs.readFileSync(path.join(__dirname, '..', '..', 'data.js'), 'utf8');
const secondaryBlock = dataJs.match(/const SECONDARY\s*=\s*\[([\s\S]*?)\];/)?.[1] ?? '';
const names          = [...new Set([...secondaryBlock.matchAll(/\["([^"]+)"/g)].map(m => m[1]))];

if (names.length === 0) {
  console.error('No secondary weapon names found in data.js — check the regex.');
  process.exit(1);
}
console.log(`Found ${names.length} secondary weapons.\n`);
