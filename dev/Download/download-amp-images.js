#!/usr/bin/env node
// Downloads amp images from the wiki into Images/amps/
// Run once: node download-amp-images.js
...
const OUT_DIR = path.join(__dirname, '..', '..', 'Images', 'amps');
fs.mkdirSync(OUT_DIR, { recursive: true });

const dataJs   = fs.readFileSync(path.join(__dirname, '..', '..', 'data.js'), 'utf8');
const ampBlock = dataJs.match(/const AMPS\s*=\s*\[([\s\S]*?)\];/)?.[1] ?? '';
const names    = [...new Set([...ampBlock.matchAll(/\["([^"]+)"/g)].map(m => m[1]))];

if (names.length === 0) {
  console.error('No amp names found in data.js — check the regex.');
  process.exit(1);
}
console.log(`Found ${names.length} amps.\n`);
